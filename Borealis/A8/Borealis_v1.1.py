...
analysis_parameters = {
   ...
   "motor_dry_mass_position": (0, 0.005),


   # === Propulsion Details ===
   # run help(SolidMotor) for more information
   ...
   # === Aerodynamic Details ===
   # run help(Rocket) for more information
   ...
   "tail_position": (2.08, 0.001),
   # Axial distance between rocket's center of dry mass and nearest point in its nose cone (m)
   "nose_position": (0, 0.001),
   # Number of fins
   "fin_number" : (3, 0),
   # Fin span (m)
   ...
   # run help(Environment) and help(Flight) for more information
...
filename = str(output_path/"Borealis")
...
for setting in flight_settings(analysis_parameters, number_of_simulations):
    ...
    # Define COTS motor
    Pro54_2014K1200 = SolidMotor(
        # Thrust data
        thrust_source=str(BASE_DIR/"simulation_inputs/propulsion_data/Cesaroni_2014K1200-16A.rse"),
    ...
    Borealis = Rocket(
        ...
        power_off_drag=str(BASE_DIR/"simulation_inputs/aerodynamic_data/BorealisA8_power_off.csv"),
        power_on_drag=str(BASE_DIR/"simulation_inputs/aerodynamic_data/BorealisA8_power_on.csv"),

        # Define the center of dry mass as the distance from the tip of the nose, and set the positive axis orientation
        center_of_mass_without_motor=1.2371,
        coordinate_system_orientation="nose_to_tail",
    )

    # Define rail buttons
    # Borealis.set_rail_buttons(
    ...
    Borealis.add_motor(Pro54_2014K1200, position=2.07)   

    # Add uncertainty to the drag curves, by multiplying them by a small, random corrective factor
    Borealis.power_off_drag *= setting["power_off_drag_corr"]
    Borealis.power_on_drag *= setting["power_on_drag_corr"]
    ...
    FinSet = Borealis.add_trapezoidal_fins(
        n=3,
