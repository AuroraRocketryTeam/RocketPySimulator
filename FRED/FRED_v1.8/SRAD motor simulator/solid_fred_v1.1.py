...
K_erosion_nozzle = 0.05   # [mm/sec * mm^02 / bar 0.8] constant for throat erosion
...
L_single_grain = ((L_cc - L_bulkhead - tp_thickness - nozzle_housing_length - L_nozzle_ring)-(n_grains+1)*grains_distance)/n_grains   # CHANGED
...
for i in range(N-1):
    ...
    R[i] = 8.31446261815324 / ( cea.get_exit_MolWt_gamma(Pc=P0[i+1], eps=exp_ratio_array[i], frozen=1, frozenAtThroat=1)[0] / 1000 )
...
def show_custom_plots():
    ...
    fig2.add_trace(go.Scatter(x=[m_design_lift_off], y=[exp_ratio_array[1]], mode='markers', name='Design Point', marker=dict(color='red', size=8)), row=1, col=3)
    fig2.add_shape(type="line", x0=m_design_lift_off, y0=0, x1=m_design_lift_off, y1=exp_ratio_array[1], line=dict(color="red", dash="dot"), row=1, col=3)
    fig2.add_shape(type="line", x0=0, y0=exp_ratio_array[1], x1=m_design_lift_off, y1=exp_ratio_array[1], line=dict(color="red", dash="dot"), row=1, col=3)
...
fig.update_yaxes(title_text="Velocity [m/sec]", row=1, col=1)
