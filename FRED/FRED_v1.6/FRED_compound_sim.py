...
std_p = 10                              # standard percentage variation
...
analysis_parameters = {
    ...
    "rocket_dry_mass": (2672 / 1000, std_gen(2672 / 1000, std_p)),
    ...
    "impulse": (impulse, std_gen(impulse, std_p)),
    # Motor burn out time (s)
    "burn_time": (burn_time, std_gen(burn_time, std_p)),
    # Motor's nozzle radius (m)
    "nozzle_radius": (13.71 / 1000, std_gen(13.71 / 1000, std_p)),
    # Motor's nozzle throat radius (m)
    "throat_radius": (9.50 / 1000, std_gen(9.50 / 1000, std_p)),
    # Motor's grain separation (axial distance between two grains) (m)
    "grain_separation": (3 / 1000, std_gen(3 / 1000, std_p)),
    # Motor's grain density (kg/m^3)
    "grain_density": (grain_density, std_gen(grain_density, std_p)),
    # Motor's grain outer radius (m)
    "grain_outer_radius": (grain_external_radius, std_gen(grain_external_radius, std_p)),
    # Motor's grain inner radius (m)
    "grain_initial_inner_radius": (grain_internal_radius, std_gen(grain_internal_radius, std_p)),
    # Motor's grain height (m)
    "grain_initial_height": (grain_length, std_gen(grain_length, std_p)),
