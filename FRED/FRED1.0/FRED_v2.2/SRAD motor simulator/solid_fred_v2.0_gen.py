...
# ---------------- MASSES MOTOR ----------------
M_motor_full = M_nozzle_ring + M_nozzle + M_tp + M_casing + M_pr 
...
# ---------------- WRITE FILE ----------------
filename = "FRED_45_PARAMETERS_8mm.txt"

with open(filename, "w") as f:
    f.write("\n".join(lines))


# ------------------------------- MSA INPUT FILE GENERATION -------------------------------
import json

# MSA input file name version
version = "FRED_45_2.1"

# Load simulation inputs
input_simulazione = {
    # Casing
    "Cs_ext_diameter": D_cc_outer,
    "Cs_thickness": th_casing,
    "Cs_length": L_cc,
    "Cs_density": rho_casing_mat,
    "Cs_yield_strength": yield_casing_mat,
    "Cs_safety_factor": SF_casing,

    # Bulkhead
    "Bkh_length": L_bulkhead,
    "Bkh_fillet_thickness": th_bulkhead_fillet,

    # Thermal protection
    "Th_thickness": tp_thickness,
    "Th_density": rho_phenolic,

    # Nozzle
    "Nz_exp_ratio": exp_ratio,
    "Nz_throath_diameter": D_throat,
    "Nz_conv_angle": conv_angle,
    "Nz_div_angle": div_angle,
    "Nz_density": rho_nozzle_mat,
    "Nz_ring_length": L_nozzle_ring,
    "Nz_housing_length": nozzle_housing_length,
    "Nz_exit_thickness": th_nozzle_exit,
    "Nz_conv_thickness_to_wall": convergent_nozzle_thickness_to_wall,
    "Nz_casing_residual": casing_residual,
    "Nz_casing_erosion_nozzle": K_erosion_nozzle,

    # O-rings
    "Or_thickness": th_oring,
    "Or_diameter": D_oring,

    # Grain geometry
    "G_int_diameter": D_int,
    "G_number": n_grains,
    "G_distance": grains_distance,
}

# Qua ho messo solo il nome del file, specifica qua la directory dove vuoi salvare il .json
nome_file = str(f"{version}_input_simulazione.json")

with open(nome_file, "w") as f:
    # indent=4 rende il file leggibile anche aprendolo con un editor di testo
    json.dump(input_simulazione, f, indent=4)

print(f"Dati salvati correttamente in {nome_file}")