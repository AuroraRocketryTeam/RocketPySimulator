...
try:
    ...
    print("Simulation started using custom_atmosphere (EuRoC Mean Data).")
except FileNotFoundError:
    print("WARNING: EuRoC JSON file not found. Switching to standard atmosphere to allow execution.")
...
sensor_position = Atlas.center_of_mass_without_motor
...
Atlas.add_sensor(accel_noisy, sensor_position)
Atlas.add_sensor(accel_clean, sensor_position)
...
Atlas.add_sensor(barom_noisy, sensor_position)
Atlas.add_sensor(barom_clean, sensor_position)
...
def save_acceleration_plots(accel_noisy, accel_clean, outdir=BASE_DIR/"plots", show=False):
    outdir = Path(outdir)
...
def save_barometer_plot(barom_noisy, barom_clean, test_flight, outdir=BASE_DIR/"plots", show=False):
    outdir = Path(outdir)
    outdir.mkdir(parents=True, exist_ok=True)
...
def save_all_plots(accel_noisy, accel_clean, barom_noisy, barom_clean, test_flight, outdir=BASE_DIR/"plots", show=False):
